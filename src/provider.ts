...
async function getGlobalMarketReport(runtime: IAgentRuntime): Promise<string> {
  try {
    ...
    return [
      ...
      btc ? `BTC Price: ${formatCurrency('$', btc.price)} (${formatUpDown(btc.percent_change_24h)} 24h | ${formatUpDown(btc.percent_change_7d)} 7d | ${formatUpDown(btc.percent_change_30d)} 30d) | BTC Dominance: ${metrics.btc_dominance.toFixed(2)}% (${formatUpDown(metrics.btc_dominance_24h_percentage_change)} 24h)` : 'BTC: N/A',
      eth ? `ETH Price: ${formatCurrency('$', eth.price)} (${formatUpDown(eth.percent_change_24h)} 24h | ${formatUpDown(eth.percent_change_7d)} 7d | ${formatUpDown(eth.percent_change_30d)} 30d) | ETH Dominance: ${metrics.eth_dominance.toFixed(2)}% (${formatUpDown(metrics.eth_dominance_24h_percentage_change)} 24h)` : 'ETH: N/A',
      sol ? `SOL Price: ${formatCurrency('$', sol.price)} (${formatUpDown(sol.percent_change_24h)} 24h | ${formatUpDown(sol.percent_change_7d)} 7d | ${formatUpDown(sol.percent_change_30d)} 30d)` : 'SOL: N/A',
      fearGreed ? `Fear & Greed: ${fearGreed.value} (${fearGreed.value_classification})` : 'Fear & Greed: N/A',
    ].join('\n');
  } catch (error) {
...
async function getTokenReports(runtime: IAgentRuntime, message: Memory, state?: State): Promise<string[]> {
  const tokens = await getTokensFromContext(runtime, message, state)
    .then((tokens) => Promise.all(
      tokens.map((token) => token.symbol ? token : figureOutTokenSymbol(runtime, token.name)),
    ))
    .catch((error) => {
  ...
  return Array.from(map.values().map(({ info, price, about }) => {
    try {
      return generateReport(info, price, about);
    } catch (error) {
      elizaLogger.error(`Failed to generate report for ${info.name} (${info.symbol}): ${error}`);
