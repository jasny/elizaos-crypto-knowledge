...
export class GlobalTokenKnowledge extends Service {
  ...
  async initialize(runtime: IAgentRuntime): Promise<void> {
    ...
    setInterval(
      () => this.update(runtime).catch((error) => elizaLogger.error(`Failed to update CoinMarketCap top ${this.top}: ${error}`)
  ...
  async update(runtime: IAgentRuntime): Promise<void> {
    ...
    const promises = this.uniqueSymbols(tokens).map((token) => ([
      ...
      limit(() => this.addMemory(runtime, stringToUuid(token.id), `${token.name} with symbol ${token.symbol} has rank ${token.rank} on CoinMarketCap`, now)),
    ])).flat();
  ...
  private async addMemory(runtime: IAgentRuntime, id: UUID, text: string, now: Date): Promise<void> {
  ...
  private async preloadGlobalMetrics(): Promise<void> {
    ...
    await Promise.all([
      this.cmc.globalMetrics({ refresh: true }).catch((error) => elizaLogger.error(`Failed to preload global metrics: ${error}`)),
      this.cmc.fearAndGreed({ refresh: true }).catch((error) => elizaLogger.error(`Failed to preload fear and greed index: ${error}`)),
      this.cmc.price(['BTC', 'ETH', 'SOL']).catch((error) => elizaLogger.error(`Failed to preload prices: ${error}`)),
    ]);

    setInterval(() => {
      this.cmc.globalMetrics({ refresh: true }).catch((error) => elizaLogger.error(`Failed to preload global metrics: ${error}`)).then();
      this.cmc.fearAndGreed({ refresh: true }).catch((error) => elizaLogger.error(`Failed to preload fear and greed index: ${error}`)).then();
