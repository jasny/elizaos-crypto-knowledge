...
export class CoinMarketCap extends Service {
    ...
    async initialize(runtime: IAgentRuntime): Promise<void> {
        ...
        this.baseUrl =
            runtime.getSetting('COINMARKETCAP_API_URL') ||
    ...
    async list(limit: number): Promise<BasicInfo[]> {
        ...
        const data = await this.get<Array<BasicInfo & { [_: string]: any }>>(
            '/v1/cryptocurrency/map',
            params
        );

        return data.map(
            ({ id, rank, name, symbol, slug }): BasicInfo => ({ id, rank, name, symbol, slug })
        );
    }
    ...
    private mergeTags(info: any) {
        ...
        info['tag-names'] = undefined;
        info['tag-groups'] = undefined;
    }
    ...
    private async cached<T extends { id: number; symbol: string }>(
        ...
        const remaining = result.filter((info) => !('id' in info)).map((info) => info.symbol);
        ...
        await Promise.all(
            newInfo.map((info) =>
                this.cache.set(`coinmarketcap:${key}:${info.symbol}`, info, options)
            )
        );
    ...
    async globalMetrics(options: { refresh?: boolean } = {}): Promise<GlobalMetrics> {
        const cached = !options.refresh
            ? await this.cache.get<GlobalMetrics>('coinmarketcap:global-metrics')
            : null;
    ...
    async fearAndGreed(options: { refresh?: boolean } = {}): Promise<FearAndGreed> {
        const cached = !options.refresh
            ? await this.cache.get<FearAndGreed>('coinmarketcap:fear-and-greed')
            : null;
    ...
    async price(tokens: string[]): Promise<Price[]> {
        return this.cached(
            ...
            async (tokens) => {
                ...
                const data = await this.get<Record<string, Price[]>>(
    ...
    async about(infos: Array<{ id: number; slug: string; symbol: string }>): Promise<About[]> {
        const map = new Map<string, (typeof infos)[number]>(
            infos.map((info) => [info.symbol, info])
        );

        return this.cached('about', Array.from(map.keys()), (tokens) =>
            Promise.all(
    ...
    private async fetchAboutSection(slug: string): Promise<string> {
        ...
        const parts = aboutSection
            .find('> div > div > section > div > div > div')
            .map((_, el) => {
                const section = $(el);
                const title = section.find('h3').text().trim();
                const paragraphs = section
                    .find('p')
                    .map((_, el) => this.turndown.turndown($(el).html()))
                    .get();
